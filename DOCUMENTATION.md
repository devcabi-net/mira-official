# 📚 Documentation Index

This document provides an overview of all documentation files in the Mira Discord Bot project.

---

## 📖 User-Facing Documentation

### **README.md**
**Purpose:** Main entry point and user guide  
**Audience:** End users, server administrators, setup guides  
**Contents:**
- Bot overview and features
- Installation and setup instructions
- All available commands
- Configuration guide
- Support information

**When to read:** Start here for general bot information and setup.

---

### **SOCIAL_CREDITS_SYSTEM.md**
**Purpose:** Comprehensive guide to the Social Credits economy system  
**Audience:** End users, community members  
**Contents:**
- How Social Credits work
- Earning credits
- Tier system explanation
- Available moderation actions
- Pricing and costs
- Protection system
- Usage tips

**When to read:** When users want to understand the Social Credits system in detail.

---

## 👨‍💻 Developer Documentation

### **DEVELOPMENT.md**
**Purpose:** Technical documentation for developers  
**Audience:** Developers, maintainers, contributors  
**Contents:**
- Architecture overview
- Project structure
- Development setup
- How to add new features
- Data storage details
- Configuration reference
- Known issues and technical debt
- Testing guidelines
- Deployment procedures
- Code style guidelines

**When to read:** When developing, maintaining, or contributing to the bot.

---

### **BOT_REVIEW.md**
**Purpose:** Historical technical audit log  
**Audience:** Developers, project managers  
**Contents:**
- Technical review findings
- Known issues (historical)
- Performance analysis
- Security review
- Code quality assessment
- Action items

**When to read:** To understand past technical decisions and issues (historical reference).

---

## 📋 Quick Reference

### **For Users:**
1. Start with **README.md** for setup and basic usage
2. Read **SOCIAL_CREDITS_SYSTEM.md** for economy details

### **For Developers:**
1. Read **README.md** for project overview
2. Read **DEVELOPMENT.md** for technical details
3. Reference **BOT_REVIEW.md** for historical context

---

## 🔄 Documentation Maintenance

**Last Updated:** 2025-01-26

**Update Frequency:**
- README.md - Update when features change
- SOCIAL_CREDITS_SYSTEM.md - Update when economy changes
- DEVELOPMENT.md - Update when architecture changes
- BOT_REVIEW.md - Historical record (update when major reviews done)

---

## ✅ Documentation Standards

- **User docs** should be clear, non-technical, and example-rich
- **Developer docs** should be technical, detailed, and code-focused
- All docs should be kept up-to-date with code changes
- Use consistent formatting and structure across docs

---

*This index is maintained alongside the codebase. Keep documentation aligned with actual bot functionality.*

